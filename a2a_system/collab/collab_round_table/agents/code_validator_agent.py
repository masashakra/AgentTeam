...
AGENT_PORT = int(os.getenv("CODE_VALIDATOR_PORT", "8015"))
AGENT_HOST = os.getenv("AGENT_HOST", "localhost")
AGENT_URL = os.getenv("CODE_VALIDATOR_URL", f"http://{AGENT_HOST}:{AGENT_PORT}")
...
class CodeValidatorAgent(BaseAgent):
    ...
    async def handle(self, message: Message, task_id: str) -> list[Artifact]:
        ...
        return [text_artifact("validation_report", json.dumps(report, indent=2))]
...
if __name__ == "__main__":
    uvicorn.run("agents.code_validator_agent:app", host="0.0.0.0", port=8015, reload=False)
